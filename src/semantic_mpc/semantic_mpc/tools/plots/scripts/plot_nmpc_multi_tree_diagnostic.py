...
def realized_measurement_likelihoods(pose, trees, true_classes, sensor, params):
    ...
    outputs = np.asarray(sensor(ca.DM(features)), dtype=float).reshape(len(trees), 2, 2)
...
def main():
    ...
    parser.add_argument("--active-targets", type=int, default=5)
    ...
    parser.add_argument("--information-gain-weight", type=float, default=10.)
    parser.add_argument("--entropy-time-pressure-weight", type=float, default=0.0)
    parser.add_argument("--exploration-weight", type=float, default=0.05)
    parser.add_argument("--attraction-weight", type=float, default=0.0)
