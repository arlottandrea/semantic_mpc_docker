...
def find_best_mlp_viewpoint(target, params, checkpoint, weights_cache, grid_size=81, yaw_steps=37):
    ...
    for relative_yaw in np.linspace(-np.pi, np.pi, int(yaw_steps), endpoint=False):
        ...
        candidate = {
            ...
            "raw_correct": float(outputs[best_index, 1]),
            "ripe_correct": float(outputs[best_index, 5]),
        }
...
def update_belief_from_pose(belief, pose, target, sensor, true_class, camera_yaw_offset=0.0):
    ...
    category_labels = ["nothing", "raw", "ripe"]
    ...
    output = np.asarray(sensor(features), dtype=float).reshape(2, 3)
...
def expected_entropy_from_mlp_outputs(outputs, prior):
    outputs = np.asarray(outputs, dtype=float).reshape(-1, 2, 3)
    ...
    for observation in range(3):
        ...
        if observation == 0:
            entropy += probability * prior_entropy
            continue
...
def plot_mlp_heatmap(path, data, grid_size=121, radius=6.0, dataset_points=None):
    ...
    raw_correct = outputs[:, 1].reshape(grid_size, grid_size)
    ripe_correct = outputs[:, 5].reshape(grid_size, grid_size)
        ...
        frames.append(
            {
                ...
                "raw": np.round(outputs[:, 1], 4).tolist(),
                "ripe": np.round(outputs[:, 5], 4).tolist(),
                "entropy": np.round(
