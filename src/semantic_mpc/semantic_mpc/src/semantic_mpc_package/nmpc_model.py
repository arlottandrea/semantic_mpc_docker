...
def _is_compatible_checkpoint(path, params):
    ...
    if metadata.get("model_type") != params.get("nn_model_type", "class_conditioned"):
        return False
    labels = metadata.get("output_labels")
    expected_labels = params.get("nn_checkpoint_output_labels", params.get("nn_output_labels"))
...
def load_l4casadi_model(params):
    ...
    from semantic_mpc_package.perception_model import ClassConditionedMLP, PoseLikelihoodMLP

    torch.jit.set_fusion_strategy([("STATIC", 0)])
    conditioned = ClassConditionedMLP(
        hidden_size=int(params["hidden_size"]), hidden_layers=int(params["hidden_layers"]),
    ...
    conditioned.load_state_dict(torch.load(get_latest_best_model(params), map_location=device))
    model = PoseLikelihoodMLP(conditioned)
    model.eval()
