...
class NmpcOptimizerTest(unittest.TestCase):
    ...
    def test_structured_output_expands_to_binary_likelihoods(self):
        ...
        np.testing.assert_allclose(np.asarray(ripe), [[0.2, 0.8], [0.5, 0.5]])
        np.testing.assert_allclose(np.asarray(raw), [[0.7, 0.3], [0.5, 0.5]])
    ...
    def test_realized_observations_select_conditional_likelihood_columns(self):
        ...
        np.testing.assert_array_equal(categories, [-1, 0, 1])
        model = np.array(
            [
                [0.8, 0.2, 0.3, 0.7],
                [0.7, 0.3, 0.2, 0.8],
    ...
    def test_optimizer_builds_and_attracts_only_outside_observation_range(self):
        ...
        uninformative = ca.Function(
            ...
            [model_input],
            [ca.repmat(ca.DM([[0.5, 0.5, 0.5, 0.5]]), batch_size, 1)],
        )
        params = {
            "state_dim": 3,
            "control_dim": 3,
    ...
    def test_active_target_respects_safe_distance_constraint(self):
        ...
        uninformative = ca.Function(
            ...
            [model_input],
            [ca.repmat(ca.DM([[0.5, 0.5, 0.5, 0.5]]), horizon, 1)],
        )
        params = {
            "state_dim": 3,
            "control_dim": 3,
