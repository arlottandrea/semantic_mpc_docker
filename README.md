...
The command recursively discovers each `run.json` and its adjacent
`control_metrics.csv`. On the host, the generated report is available at
`runs/reports/latest/`. Use a different directory for `--output-dir` to preserve an
older report.

W&B is optional. To upload controller runs, set `wandb_enabled: true` and
`wandb_mode: "online"` in
`src/semantic_mpc/semantic_mpc/config/experiment.yaml`, and set `WANDB_API_KEY` in the
root `.env` file before running the controller batches. Local `run.json` and CSV
telemetry is still written when W&B is disabled or offline.

To build a report by downloading runs that are already present in W&B, use
`--source wandb`. Repeat `--project` for every `entity/project` to compare. The default
filter includes only finished runs; use `--state ""` to include every state and repeat
`--tag TAG` to require tags.
...
Offline W&B runs must be synced separately before the W&B command can download them.
If a report export has already been copied to the machine, it can be regenerated without
accessing either raw local runs or W&B:

```bash
docker compose run --rm --no-deps --entrypoint python baseline \
  /workspace/scripts/generate_wandb_report.py \
  --source export \
  --input-dir /runs/reports/previous \
  --output-dir /runs/reports/regenerated
```

The input directory for export mode must contain `runs.csv`; `history.csv` is optional.

Every report contains CSV tables, `batch_report.xlsx`, `REPORT.md`, and the plots that
can be produced from the available history (metric box plots, tree-entropy plots, and
normalized entropy/velocity profiles). Check
`fairness_audit.csv` (or the matching XLSX sheet) before interpreting rankings. It
compares the effective control and sampling periods, velocity and acceleration limits,
and obstacle clearance recorded for the runs. Any required `FAIL` invalidates the
ranking. `pairing_audit.csv` requires exactly one run per algorithm and trial seed;
summary tables and plots use complete pairs only when complete pairs are available.
