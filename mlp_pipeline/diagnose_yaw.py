...
def mutual_information(likelihood):
    ...
    likelihood = np.asarray(likelihood, dtype=float).reshape(-1, 2, 3)
...
def assemble_empirical_rows(features, targets):
    ...
    for feature, target in zip(features, targets):
        ...
        item = by_pose.setdefault(key, np.full((2, 3), np.nan))
        class_index = 0 if target[3] > 0.5 else 1
        selected = select_model_targets(target[None, :], {"output_dim": 6})[0]
        item[class_index] = selected.reshape(2, 3)[class_index]
    keys, matrices = [], []
...
def main():
    ...
    model = MultiLayerPerceptron(
        ...
        hidden_layers=int(cfg["hidden_layers"]), output_dim=6,
    ...
    prediction = prediction_tensor.detach().numpy().reshape(-1, 2, 3)
    ...
    rows = prediction_tensor.reshape(-1, 2, 3)
    joint = 0.5 * rows
